...
def generate_quiz(lecture_slides, chapter, quiz_name, path, user, share, youtube_link, audio_video, email, name, url):
    with app.app_context():
        ...
        if lecture_slides != "":
            ...
            db.session.query(QuizNames).filter_by(id=new_quiz_name.id).first().func_prog = 10
            db.session.commit()
            main_quiz.paragraph(words.transcribe(words.path))
            ...
            db.session.query(QuizNames).filter_by(id=new_quiz_name.id).first().func_prog = 10
            db.session.commit()
            main_quiz.paragraph(words.transcribe(path))
            os.remove(path)
